...
def main():
  ...
  ofile  = open('../results/errorDataVaryPrecip.csv', "wb")
  writer = csv.writer(ofile, delimiter=',')
  writer.writerow(["NumberOfBins"]+["Precip (> mean of 0.089)"]+["PredictedOctoberError"]+["BaselineOctoberError"]+["SpecificConditionsVsAllCrimesOctoberError"])
  

  for number_of_bins in bins_list:
    for precip_level in levels:
    ...
    # #*******************************************************************************************
      ...
      for train in data:
        if(int(train.weather.precipitation>0.089)==precip_level):
          training_set.append(train)

      for test in data2:
        if(int(test.weather.precipitation>0.089)==precip_level):
      ...
      # writer.writerow("NumberOfBins,TypeOfCrime,PredictedOctoberError,BaselineOctoberError,SpecificConditionsVsAllCrimesOctoberError")
      writer.writerow([str(number_of_bins)]+[str(precip_level)]+[str(heatmapError(true_map, predict_map))]+
        [str(heatmapError(true_map, average_map))]+[str(heatmapError(all_crimes_map,predict_map))])

      drawHeatmap(predict_map, "../results/predictPrecip"+str(number_of_bins)+"_"+str(precip_level)+"_"+".png")
      drawHeatmap(true_map, "../results/truePrecip"+str(number_of_bins)+"_"+str(precip_level)+"_"+".png")
      drawHeatmap(average_map, "../results/baselinePrecip"+str(number_of_bins)+"_"+str(precip_level)+"_"+".png")
      drawHeatmap(all_crimes_map, "../results/allcrimesPrecip"+str(number_of_bins)+"_"+str(precip_level)+"_"+".png")

      print "Prediction error on test (type of crime:  ",str(precip_level),") (num_bins: ",str(number_of_bins),"): ",heatmapError(true_map, predict_map)
